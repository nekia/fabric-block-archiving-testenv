...
function generateChannelArtifacts() {
  ...
  mkdir ./channel-artifacts/$CHANNEL_NAME
  echo
  echo "#################################################################"
  ...
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/$CHANNEL_NAME/Org2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org2MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
elif [ "$MODE" == "generatech" ]; then
  EXPMODE="Generating genesis block for specified channel"
else
  printHelp
  exit 1
fi

while getopts "h?c:t:d:s:l:i:anvr" opt; do
  case "$opt" in
  h | \?)
    printHelp
    exit 0
...
if [ "${MODE}" == "up" ]; then
  ...
  upgradeNetwork
elif [ "${MODE}" == "generatech" ]; then ## Upgrade the network from version 1.2.x to 1.3.x
  generateChannelArtifacts
else
  printHelp
  exit 1
